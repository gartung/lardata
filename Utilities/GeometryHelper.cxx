#ifndef UTIL_GEOMETRYHELPER_CXX
#define UTIL_GEOMETRYHELPER_CXX
...
void GeometryHelper::Reconfigure()
{
  ...
  // auto geom = util::Geometry::GetME();
  // Need detector properties and lar properties for time to cm conversion
  // auto detp = util::DetectorProperties::GetME();
  // auto larp = util::LArProperties::GetME();
...
Point2D GeometryHelper::Point_3Dto2D(const TVector3 & _3D_position, unsigned int plane) const {
  ...
  if (plane > geom -> Nplanes()) {
    throw util::UtilException(Form("Can't project 3D point to unknown plane %u", plane));
  }
  ...
  ///\TODO: Convert the plane to planeID
  returnPoint.w = geom->WireCoordinate(_3D_position.Y(), _3D_position.Z(), plane,0,0) * fWireToCm;
...
float GeometryHelper::Slope_3Dto2D(const TVector3 & inputVector, unsigned int plane) const {
  ...
  TVector3 GeometryHelper::Project_3DLine_OnPlane(const TVector3& inputVector, const int& pl) const {
    ...
    if (pl == 0)
      ...
      throw UtilException("Invalid plane! this plane number does not exist");
  ...
  std::vector<double> GeometryHelper::Project_3DLine_OnPlane(const std::vector<double>& V, const std::vector<double>& N) const {
    ...
    if ( (V.size() != 3) or (N.size() != 3) )
      throw UtilException("Project_3DLine_OnPlane failed due to unrecognized vector size");
...
double GeometryHelper::GetPitch(const TVector3& direction, const int& pl) const
{
  ...
  if (pl == 0)
    ...
    throw UtilException("Plane number out of bounds!");
...
double GeometryHelper::GetCosAngleBetweenLines(const Point2D& p1, const Point2D& p2, const Point2D& p3) const
{
  ...
  if ( ( (p1.w == p2.w) and (p1.t == p2.t) ) or
    ...
    throw UtilException("Trying to calculate dot-product using a zero-length vector!");
    ...
    double frac) const
{
  ...
  std::vector<util::Point2D> edges(4, Point2D(plane, 0, 0));
  ...
  for (size_t index = 0; index < ordered_hits.size(); ++index) {
    ...
    if (ordered_hits.at(index)->t < -detp -> TriggerOffset() ||
      ...
      throw UtilException(Form("Invalid wire/time (%g,%g) for plane %i ... range is (0=>%g,0=>%g)",
